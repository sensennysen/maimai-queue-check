...
```bash
INIT=$(node "E:/git/smf-queue-check/.codex/get-shit-done/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
...
```bash
node "E:/git/smf-queue-check/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
