...
- **Global scope**: `E:/git/smf-queue-check/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/git/smf-queue-check/.codex/notes/`)
...
2. Glob `E:/git/smf-queue-check/.codex/notes/*.md` (if directory exists) — global notes
...
Global (E:/git/smf-queue-check/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/git/smf-queue-check/.codex/notes/` — works in any directory
