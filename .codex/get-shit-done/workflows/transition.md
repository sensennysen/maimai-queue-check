...
```bash
TRANSITION=$(node "E:/git/smf-queue-check/.codex/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "E:/git/smf-queue-check/.codex/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
node "E:/git/smf-queue-check/.codex/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
