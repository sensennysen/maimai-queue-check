...
```bash
INIT=$(node "E:/git/smf-queue-check/.opencode/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "E:/git/smf-queue-check/.opencode/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
